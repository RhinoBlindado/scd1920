...
unsigned
   ...
   cont_cons[num_items], // contadores de verificación: consumido
   cont_prod_2[num_prod] = {0};


int step_p = num_items/num_prod;
...
int producir_dato( int i)
{
   ...
   mtx.lock();
   cout << i<<" producido: " << cont_prod_2[i]+i*step_p << endl << flush ;
   cout << "Total hebra "<< i <<" :" << cont_prod_2[i]  << endl;
   mtx.unlock();
   cont_prod_2[i]++;
   cont_prod[contador] ++ ;
   contador++;
   return (cont_prod_2[i]+i*step_p)-1;
}
//----------------------------------------------------------------------
...
class ProdCons1SC : public HoareMonitor
{
 ...
 int                        // variables permanentes
   ...
   primera_ocupada;       // indice de celca de proxima lectura
...
void funcion_hebra_productora( MRef<ProdCons1SC> monitor, int prod )
{ 
   for( unsigned i = 0 ; i < step_p ; i++ )
   {
      int valor = producir_dato( prod ) ;
      monitor->escribir( valor );
   }
}
// -----------------------------------------------------------------------------

void funcion_hebra_consumidora( MRef<ProdCons1SC> monitor, int cons )
{
   int step = num_items/num_cons;
   for( unsigned i = 0 ; i < step ; i++ )
   {
      int valor = monitor->leer();
      consumir_dato( valor, cons ) ;
   }
}
// -----------------------------------------------------------------------------
